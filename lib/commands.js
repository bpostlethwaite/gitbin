...
var commands = function () {
    ...
    that.add = function (cb) {
        ...
        if (files.length === 0) {
            ...
            cb( errf('<Add> command requires file arguments', true) , null);
        }
        else {
            var bin = getActiveBin( getState() );
            var cp = function (arg, cb_) {
                var argpath = path.join(process.cwd(),arg);
...
//
// ## PARSESWITCH ##
// Main command parse and router
//
    that.go = function (command, args, state, cb) {
//
// Set commands internal state
//
        my.args = args;
        my.state = state;
//
...
//
        ...
        case 'bin':
            ...
            break;
        default:
            cb( errf( ((command === undefined) ? '' : 'Unknown command: ' + command), true), null );
        }
    };
                ...
                if (item in state[i].binfiles) {
                    if (inState) cb(null); // Yes we found item and it's good
                        ...
                        cb( errf('Item: ' + e.blue + item
                                 + e.reset + ' is already being tracked.', false), null ); 
                    }
                }
                else {
                    if (inState) { // item not found and it is bad
                        cb( errf('Item: ' + e.blue + item 
                                 + e.reset + 'is not in the tracked list.', false), null );
                    }
    ...
    var checkItem = function (item, type, cb) {
        fs.stat(item, function (error, stats) {
            if (type !== 'f' && type !== 'd') {
                ...
                cb( errf('File error with ' + e.blue + item + e.reset, false) );
                         
            } // Probably need to mention something about improper bin pointer
            else if ( (type === 'd') && (error || !( stats.isDirectory())) ) {
                cb( errf('Directory error with ' + e.blue + item + e.reset, false) );
            }
            else cb( null );
        }); // end fs.stat
    }; // end checkFile
