...
//
    ...
    that.readstate = function ( cb ) {
        futil.readJSON(this.statefile, function (err, state) {
            if (err) {
                cb( errf('Problem reading file ' + e.blue
                         + this.statefile + e.red 
                         + 'Delete file or replace with a backup' + e.reset));
            }
            else cb(null, state);
        });
    }; // end readstate
    ...
    that.writestate = function (state, cb) {
        futil.writeJSON (this.statefile, state, function(err) {
            if (err) {
                cb( errf('Problem writing to file' + e.blue
                         + this.statefile + e.reset ))
            }
        });
    }; // end writestate
