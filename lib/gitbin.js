...
// Main command router
//
  that.run = function (command, args, cbm) {
    if (typeof cbm !== 'function') {
      var cbm = cbmain
    }
    my.args = args
    my.command = command
  ...
  function init (cb ) {
    ...
    getState( function (err) {
      ...
      if (my.state.bins[newbin]) {
        return cb( erf( 'Bin ' + e.blue + path.basename(bin) 
                        + e.reset + ' already initialized') )
      }
      ...
      writeJSON( path.join(newbin,that.flocal), '{}', function (err) {
        if (err) {
          return cb(erf('Problem writing to local state file: '
                        + e.blue + newbin + '/' + that.flocal + e.reset) )
        }
...
// Check for file arguements
    if (files.length === 0) {
      return cb( erf ('<remove> command requires file arguements', true))
    }
// Get bin and run chained processes
    getState( function (err) {
      if (err) return cb(err)  //READ FILE ERRORS
      async.asyncMap( files, processchain , function (err) {
        if (err) return cb(err)  //ERRORS FROM PROCESSING CHAIN
         writeJSON(my.state.binfile, my.state.trackedfiles, function (err) {
          if (err) {
...
// Parse for flags    
    if (my.args.length === 0) {
      ...
      else return cb( erf('Unknown flag: ' 
                          + e.red + flag + e.reset) )
    }
    function binlist() {
      var bin
    ...
    function deletebin(delbin) {
      ...
      getState( function (err) {
        ...
        if (!found) {
          return cb( erf( e.blue + bin + e.reset
                          + ' not an initialized bin'))
        }
        cb(null)
      })
    } // end deletebin
...
//
  function getState (cb) {
    ...
    readJSON( that.fglobal, function (err, bins) {
      ...
      else return cb( erf('No active bin. Set with <checkout> <bin> command') )
    })  // end readJSON
  }  // end getState
