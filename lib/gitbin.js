...
// error argument.
  function checkState ( item, wantIt, cb) {
    ...
    if (found && !wantIt) {
      return cb( erf('Item: ' + path.basename(item)
...
//
  function readState (cb) {
    ...
    fs.readFile(that.fglobal, 'utf8', function (err, data) {
      if (err) {
        return cb( erf('Problem reading global state file: ' + that.fglobal) )
      }
      try {
...
/// PUT THIS LOGIC OUTSIDE OF READ FUNCTION! ///////////////////////
...
////////////////////////////////////////////////////////////
      ...
      if (activebin) {
        ...
        stats(localbinfile, 'isFile', function (err) {
          if (err) {
            return cb( erf('Problem loading tracked file list '
                           + state.binfile
